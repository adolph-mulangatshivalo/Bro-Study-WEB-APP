...
async function init() {
  ...
  try {
    ...
    // Draw initial frame to unlock the stream
    pipCtx.save();
    pipCtx.scale(-1, 1);
    pipCtx.drawImage(webcam, -pipCanvas.width, 0, pipCanvas.width, pipCanvas.height);
    pipCtx.restore();
    
    // Feed flipped canvas stream to pip video
    ...
    // Test for popup permissions right as they enter the site
    const testPopup = window.open('about:blank', 'TestPopup', 'width=100,height=100');
    if (!testPopup) {
      alert("⚠️ POPUPS ARE BLOCKED! ⚠️\n\nPlease look at the right side of your address bar, click the small 'Popup blocked' icon, and select 'Always allow popups'. Then refresh the page!");
    } else {
      testPopup.close();
    }
    ...
    alert("Failed to access webcam or load AI model. Please ensure you granted camera permissions.");
  }
}
...
function spawnMemes() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    overlapping = true;
    while (overlapping && attempts < 50) {
      popLeft = Math.floor(Math.random() * (screenW / 2)) + (screenW / 2) - memeW;
